...
def plot_confusion_matrix(y_true, y_pred, classes, title='Confusion Matrix'):
    ...
    plt.title('Deeploc 10 Class Confusion Matrix', fontsize=16, fontweight='bold')
...
if __name__ == "__main__":
    # Load data
    clf_data_path = 'classification/Deeploc/deeploc_data.fasta'
    ...
    selected_labels = ['Nucleus', 'Cytoplasm', 'Extracellular', 'Mitochondrion']

    for record in SeqIO.parse(clf_data_path, 'fasta'):
        header = str(record.description)
        label = header.split(' ')[1]
        if label not in subclass_to_parent:
            continue

        label = subclass_to_parent[label]
    ...
    dim = 1024  # Dimension for embedding
    ...
    # XGBoost parameters - Changed to multi:softmax
    xgb_params = {
        'objective': 'multi:softmax',  # Changed from 'multi:softprob'
        ...
        'tree_method': 'hist',
        'random_state': 31,
        'eval_metric': 'mlogloss',  # Can also use 'merror' for multi:softmax
        'verbosity': 1,
    }
    ...
    # Note: For multi:softmax, predict_proba still works if you need probabilities
    # but the primary output from predict() will be class labels directly
    try:
        y_pred_proba = xgb_classifier.predict_proba(X_test)
        print("Probability predictions available via predict_proba()")
    except:
        print("Probability predictions not available with current configuration")
        y_pred_proba = None
