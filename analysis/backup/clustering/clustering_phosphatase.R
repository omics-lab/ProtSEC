...
# python3 get_phase_dist_mat.py -n 1024 -i data/clustering/phosphatase/phosphatase.fa -o  data/clustering/phosphatase/phosphatase.ProtSEC_matrix.csv

# ---- UMAP phosphatase----

# List all CSV files in the phosphatase directory
csv_files <- list.files("phosphatase", pattern = "\\_matrix\\.csv$", full.names = TRUE)

# Extract method names from file names
method_names <- gsub("phosphatase/|phosphatase\\.|_matrix\\.csv", "", basename(csv_files))
...
combined_umap_labeled <- subset(combined_umap_labeled, !(accession %in% exclude_df$accession)) %>%
  filter(label != "")
dim(combined_umap_labeled)
unique(combined_umap_labeled$label)
...
p_combined <- ggplot(combined_umap_labeled, aes(x = V1, y = V2, color = label)) +
  ...
  labs(title = "Phosphatase", x = "UMAP 1", y = "UMAP 2", color = "Label") +
...
ggsave("../plots/combined_phosphatase_umap.pdf", plot = p_combined, width = 15, height = 10)
...
ggsave("../plots/combined_phosphatase_umap_with_trust.pdf", plot = combined_layout, width = 15, height = 10)
