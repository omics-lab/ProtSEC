...
p_phosphatase <- ggplot(umap_df_labeled, aes(x = V1, y = V2, color = label, label = label)) +
  geom_point(size = 3) +
...
print(p_phosphatase)
ggsave("../../plots/phosphatase_umap.pdf", plot = p_phosphatase, width = 7, height = 5)
...
p_kinase <- ggplot(umap_df_labeled, aes(x = V1, y = V2, color = label, label = label)) +
  geom_point(size = 3) +
  geom_text(vjust = 1.5, hjust = 1.1, size = 2.5) +
  theme_minimal() +
  scale_color_brewer(palette = "Set1") +
  labs(title = "UMAP Colored by Label", x = "UMAP 1", y = "UMAP 2", color = "Label")
print(p_kinase)
ggsave("../../plots/kinase_umap.pdf", plot = p_kinase, width = 7, height = 5)
...
p_sam <- ggplot(umap_df_labeled, aes(x = V1, y = V2, color = label, label = label)) +
  geom_point(size = 3) +
  geom_text(vjust = 1.5, hjust = 1.1, size = 2.5) +
  theme_minimal() +
  scale_color_brewer(palette = "Set1") +
...
print(p_sam)
ggsave("../../plots/sam_umap.pdf", plot = p_sam, width = 7, height = 5)
